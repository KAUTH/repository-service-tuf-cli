...
def validate_cli_add_artifact_options(
    settings: Dict[str, Any], token: Optional[str], rstuf_server: Optional[str]
) -> None:
    """
    ...
    :param rstuf_server: RSTUF API server.
    ...
    if settings.get("AUTH") is False:
        ...
        if token is None:
            ...
            if rstuf_server is None and settings.get("SERVER") is None:
                raise click.ClickException(
                    "Provide the --rstuf-server option."
                )

            # Not writing the info to the actual `.rstuf.ini` file until we
            # define its use besides for the login.
            settings.setdefault("SERVER", rstuf_server)
